#!/usr/bin/env bash
# install.sh — Install the Stride exploratory-testing bundle for OpenCode
#
# Usage:
...
# This copies the skills, commands, agents, lib/ helpers, and fixtures into the
# OpenCode discovery paths, and AGENTS.md to the project root.
...
if [ "$MODE" = "global" ]; then
  ...
  echo "Installing Stride Exploratory Testing for OpenCode into ~/.config/opencode/ (global)..."
else
  OC_DIR=".opencode"
  ROOT_DIR="."
  echo "Installing Stride Exploratory Testing for OpenCode into .opencode/ (project-local)..."
fi
...
if [ -f "$SCRIPT_DIR/AGENTS.md" ] && [ -d "$SCRIPT_DIR/skills" ]; then
  ...
  git clone --quiet --depth 1 "$REPO" "$TMPDIR/stride-opencode-exploratory-testing"
  SRC="$TMPDIR/stride-opencode-exploratory-testing"
fi
...
# Smoke-test helpers and worked fixtures live alongside under the config dir.
...
BEGIN_MARKER="<!-- BEGIN stride-exploratory-testing -->"
END_MARKER="<!-- END stride-exploratory-testing -->"
NOTE_MARKER="<!-- Managed by the stride-opencode-exploratory-testing installer; content between these markers is regenerated on each install. Add your own notes outside this block. -->"
...
echo ""
echo "Stride Exploratory Testing for OpenCode installed."
echo ""
echo "Installed into $OC_DIR:"
echo "  Skills:   $(ls -d "$OC_DIR/skills/"*/ 2>/dev/null | wc -l | tr -d ' ')"
echo "  Commands: $(ls "$OC_DIR/commands/"*.md 2>/dev/null | wc -l | tr -d ' ') (/charter, /nightmare-headline, /explore, /recon, /debrief)"
...
echo ""
echo "There is NO plugin to register in opencode.json — exploratory testing has no hooks."
echo ""
echo "Next steps:"
echo "  1. Restart OpenCode so it discovers the new commands (/charter, /explore, ...)."
echo "  2. Point a command at a running app you are authorized to test — e.g."
echo "     /charter <feature>, then /explore <feature>. The explorer never touches"
echo "     production or an unauthorized system."
