...
State that the recon is complete and, when a file was written, name the path. Point the user at the natural next step — pursue a candidate charter with `/explore`, or refine the charter list with `/charter` / `/nightmare-headline`. Do NOT auto-run a session and do NOT chain into another command.
